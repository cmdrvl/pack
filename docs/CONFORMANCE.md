...
| PCK-PROFILE-001 | Profile typing includes drafts and frozen profiles, while frozen profile identity is recorded only from declared `status: frozen` plus `profile_sha256` metadata. | `tests/seal_suite.rs::frozen_profile_identity_is_recorded_in_manifest_member`; `tests/seal_suite.rs::draft_profile_remains_profile_without_fabricated_identity`; `src/detect/member_type.rs::detects_frozen_profile_identity_without_changing_type`; `src/inspect.rs::inspect_surfaces_frozen_profile_identity` |
